 ...
 <article class="lyt pnl qdl qdh">
  ...
  <div class="cnt ">
   <amp-list  id="last-blocks" layout="flex-item" :src="'//' + slug + '.com-http.us/a/last'" items="d" class="cgl" v-html='`<template type="amp-mustache"><ul class="blck">
     {{#height}}<li><a href="block/{{height}}">{{height}}</a></li>{{/height}}
     ...
     {{#tx}}<li class="txs popover popover-left"><button>Txs</button>
     <ul class="popover-container txc"><li><a href="/explorer/tx/{{.}}">{{.}}</a></li></ul></li>{{/tx}}
